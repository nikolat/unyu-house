	...
	#getEventsPhase2 = async (filterPhase2: Filter[], filterPhase3: Filter[], goPhase3: Boolean) => {
		...
		if (goPhase3) {
			...
			if (eventsAll.length > 0) {
				const pubkeysToGet: string[] = this.#getPubkeysForFilter(eventsAll).filter(
			...
			if (newestEvent) {
				...
				if (toSet) {
					...
					if (publicList.includes(eventId)) {
						tags = tags.filter(
							(tag) => !(tag.length >= 2 && tag[0] === tagName && tag[1] === eventId)
						);
						content = newestEvent.content;
			...
			return new Promise((resolve) => {
				getGeneralEvents(rxNostr, tie, relaysToGetRelays, [
					...
					if (events.length === 0) {
						...
						const ev: NostrEvent = events.reduce((a: NostrEvent, b: NostrEvent) =>
