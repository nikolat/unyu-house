...
export class RelayConnector {
  ...
  constructor(
    rxNostr: RxNostr,
    tie: OperatorFunction<EventPacket, EventPacket & { seenOn: Set<string>; isNew: boolean }>,
    relays: string[],
    loginPubkey: string,
    filterBase: Filter[],
    until: number,
  ...
  getEventsPhase1 = async () => {
    ...
    const promises = [];
    promises.push(
      getGeneralEvents(this.#rxNostr, this.#tie, this.#relays, filterPhase1, this.#callbackEvent).then((evs: NostrEvent[]) => {
        for (const ev of evs) {
    ...
    const channelsToGet: string[] = events[42]
      .map((ev) => ev.tags.find((tag) => tag.length >= 4 && tag[0] === 'e' && tag[3] === 'root')?.at(1) ?? '')
      .filter((e) => e !== '');
    ...
    pubkeysToGet = this.#getPubkeysForFilter(Object.values(events).flat()).filter(
      (pubkey) => !events[0].map((ev) => ev.pubkey).includes(pubkey),
    );
    if (pubkeysToGet.length > 0) {
      filterPhase2.push({
        kinds: [0],
  ...
  #getEventsPhase2 = async (filterPhase2: Filter[], filterPhase3: Filter[], goPhase3: Boolean) => {
    ...
    const promises = [];
    promises.push(
      getGeneralEvents(this.#rxNostr, this.#tie, this.#relays, filterPhase2, this.#callbackEvent).then((evs: NostrEvent[]) => {
        for (const ev of evs) {
...
export const sendEditProfile = async (rxNostr: RxNostr, relaysToUse: object, loginPubkey: string, prof: Profile): Promise<void> => {
  return new Promise((resolve, reject) => {
    let newestEvent: NostrEvent;
    const onevent = (packet: EventPacket) => {
      const ev = packet.event;
...
export const sendMuteUser = async (rxNostr: RxNostr, relaysToUse: object, loginPubkey: string, pubkey: string, toSet: boolean) => {
  await sendPinOrMute(rxNostr, relaysToUse, loginPubkey, pubkey, toSet, 10000, 'p');
};

export const sendMute = async (rxNostr: RxNostr, relaysToUse: object, loginPubkey: string, eventId: string, toSet: boolean) => {
  await sendPinOrMute(rxNostr, relaysToUse, loginPubkey, eventId, toSet, 10000, 'e');
};

export const sendPin = async (rxNostr: RxNostr, relaysToUse: object, loginPubkey: string, eventId: string, toSet: boolean) => {
  await sendPinOrMute(rxNostr, relaysToUse, loginPubkey, eventId, toSet, 10005, 'e');
};
          ...
          if (publicList.includes(eventId)) {
            ...
            try {
              content = await window.nostr.nip04.encrypt(
                loginPubkey,
                JSON.stringify(list.filter((tag) => !(tag.length >= 2 && tag[0] === tagName && tag[1] === eventId))),
              );
            } catch (error) {
              reject(error);
              return;
...
export const getRelaysToUse = (
  ...
  switch (relaysSelected) {
    ...
    case 'nip07':
      if (window.nostr === undefined || window.nostr.getRelays === undefined) return Promise.resolve({});
