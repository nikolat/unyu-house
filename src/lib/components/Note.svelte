...
{#if rootId !== undefined && channel !== undefined}
  ...
  {@const isMutedNoteWord = wordList.reduce(
    (accumulator, currentValue) =>
      accumulator || note.content.includes(currentValue),
    false
  )}
  {@const isMutedChannelPubkey = muteList.includes(channel.event.pubkey)}
  {@const isMutedChannelWord = wordList.reduce(
    (accumulator, currentValue) =>
      accumulator || channel.name.includes(currentValue),
    false
  )}
  {@const isMuted =
    isMutedNotePubkey ||
    ...
    isMutedChannelWord}
  {#if !isMuted}
    {@const channelId = nip19.neventEncode({
      id: rootId,
      relays: pool.seenOn(rootId),
    ...
    <dt id="note-{note.id}">
      {#if profs[note.pubkey]}
        <img
          ...
          alt="avatar of {nip19.npubEncode(note.pubkey)}"
          width="32"
          height="32"
        />
        {profs[note.pubkey].display_name ?? ""}
        <a href="/{nip19.npubEncode(note.pubkey)}"
          >@{profs[note.pubkey]?.name ?? ""}</a
        >
      {:else}
        <img src="/default.png" alt="" width="32" height="32" /><a
          href="/{nip19.npubEncode(note.pubkey)}"
          >@{nip19.npubEncode(note.pubkey).slice(0, 10)}...</a
        >
      {/if}
      <br />
    ...
    {@const replyTags = note.tags.filter(
      (v) => v[0] === "e" && v[3] === "reply"
    )}
    {@const replyPubkeys = note.tags
      .filter((v) => v[0] === "p")
      .map((v) => v[1])}
    {@const r = getExpandTagsList(
      note.content,
      note.tags.filter((v) => v[0] === "emoji")
    )}
    ...
    {@const contentWarningTag = note.tags.filter(
    ...
    <dd>
      ...
      <button
        class="content-warning-show {contentWarningTag.length > 0
          ? ''
          : 'hide'}"
        on:click={() => showContentWarning(note.id)}>Show Content</button
      >
      <div
          ...
          {#each favList as ev}
            {#if ev.tags
              ...
              <li>
                ...
                {/if}
                <img
                  src={prof.picture || "/default.png"}
                  alt="avatar of {nip19.npubEncode(ev.pubkey)}"
                  width="16"
                  height="16"
                />
                {prof.display_name ?? ""}
                <a href="/{nip19.npubEncode(ev.pubkey)}">@{prof.name ?? ""}</a> reacted
              </li>
            {/if}
          {/each}
        </ul>
      ...
      <div class="action-bar">
        {#if loginPubkey}
          <details>
            <summary>
              <svg><use xlink:href="/arrow-bold-reply.svg#reply" /></svg><span
                >reply to @{#if profs[note.pubkey]}{profs[note.pubkey]?.name ??
                    ""}{:else}{nip19
            ...
            <textarea
              id="input-text"
              bind:value={inputText[note.id]}
              disabled={!loginPubkey}
            />
            <button
              on:click={() => {
                callSendMessage(note);
              }}
              disabled={!loginPubkey || !inputText[note.id]}>Reply</button
            >
          </details>
          <button
            class="fav"
            on:click={() => sendFav(pool, relaysToWrite, note, "+")}
            disabled={!loginPubkey}
            ><svg><use xlink:href="/heart.svg#fav" /></svg></button
          >
          <button
            class="emoji"
            on:click={() => callSendEmoji(pool, relaysToWrite, note)}
            disabled={!loginPubkey}
            ><svg><use xlink:href="/smiled.svg#emoji" /></svg></button
          >
          <div
            bind:this={emojiPicker[note.id]}
            class={visible[note.id] ? "" : "hidden"}
          />
          {#if note.pubkey === loginPubkey}
            <button
              class="delete"
              on:click={() => callSendDeletion(pool, relaysToWrite, note.id)}
              disabled={!loginPubkey || note.pubkey !== loginPubkey}
              ><svg><use xlink:href="/trash.svg#delete" /></svg></button
            >
          {/if}
        {/if}
        <details>
          <summary
            ><svg><use xlink:href="/more-horizontal.svg#more" /></svg></summary
          >
          <dl class="details">
            <dt>User ID</dt>
            ...
            <dd>
              <pre class="json-view"><code
                  >{JSON.stringify(note, undefined, 2)}</code
                ></pre>
            </dd>
