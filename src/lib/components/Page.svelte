<script lang="ts">
	...
	const callbackEvent = async (event: NostrEvent, redraw: boolean = true) => {
		...
		if (
			eventsAll.some(
				(ev) =>
					[0, 3, 41, 10000, 10005, 10030].includes(ev.kind) &&
					ev.kind === event.kind &&
		...
		switch (event.kind) {
			...
			case 7: {
				...
				const targetChannel7 = channels.find(
					(channel) =>
						channel.event.id ===
						event.tags.find((tag) => tag.length >= 4 && tag[0] === 'e' && tag[3] === 'root')?.at(1)
				);
			...
			case 42: {
				...
				if (currentChannelId) {
					...
					isQuote = !event.tags.some(
						(tag) => tag.length >= 2 && tag[0] === 't' && tag[1] === currentHashtag
					);
				} else if (currentEvent) {
	...
	const getListWithEncrypt = async (event: NostrEvent, tagName: string): Promise<string[]> => {
		let rList =
			event.tags.filter((tag) => tag.length >= 2 && tag[0] === tagName).map((tag) => tag[1]) ?? [];
		...
		if (isLoggedin && loginPubkey && event.content && browser && nostr?.nip04?.decrypt) {
			try {
				...
				rList = rList.concat(
					list.filter((tag) => tag.length >= 2 && tag[0] === tagName).map((tag) => tag[1])
				);
			} catch (error) {
				console.warn(error);
			}
	...
	const applyRelays = async () => {
		...
		if (isLoggedin) {
			eventCopy = [
				...eventCopy,
				...eventsAll.filter((ev) => [3, 10000, 10005, 10030, 30007, 30030].includes(ev.kind))
			];
		}
