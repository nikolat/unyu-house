<script lang="ts">
	...
	const callSendMessage = (noteToReply: NostrEvent) => {
		...
		if (!content) return;
		inputText[noteId] = '';
		const details: HTMLDetailsElement = document.querySelector(
			`#note-${noteId} + dd div.action-bar details`
		)!;
		details.open = false;
		resetScroll();
		...
		{@const noteOrg =
			note.kind === 42
				...
				: [...notes, ...notesQuoted].find(
						(ev) => ev.id === note.tags.find((tag) => tag.length >= 2 && tag[0] === 'e')?.at(1)
					)}
				...
				<dt id="note-{note.id}">
					{#if note.kind === 16}
						...
						{#if profs[note.pubkey]}
							<img
								src={profs[note.pubkey].picture || getDefaultAvatar(note.pubkey, 16)}
								alt="avatar of {npub}"
								width="16"
								height="16"
							/>
						...
						<div class="content">
							{plainTexts[0]}{#each Array.from(matchesIterator) as match, i (i)}{#if /https?:\/\/\S+/.test(match[1])}<a
									...
									/>{:else if /nostr:naddr\w+/.test(match[6])}{@const matchedText =
										match[6]}{@const naddrText = matchedText.replace(/nostr:/, '')}<a
					...
					{#if favList.some((ev) => ev.tags
							...
							{#each favList as ev (ev.id)}{#if ev.tags
									...
									?.at(1) === noteOrg.id && profs[ev.pubkey] && !muteList.includes(ev.pubkey) && !muteListFav.includes(ev.pubkey)}{@const emojiTag =
										...
										profs[ev.pubkey]}{@const npubFaved = nip19.npubEncode(ev.pubkey)}
									<li>
						...
						{#if isLoggedin}<details>
								<summary>
									<svg><use xlink:href="/arrow-bold-reply.svg#reply"></use></svg><span
										>reply to @{#if profs[noteOrg.pubkey]}{profs[noteOrg.pubkey]?.name ??
												''}{:else}{npubOrg.slice(0, 10)}...{/if}</span
									>
								</summary>
								<textarea
									name="input-text"
